...
router.get('/', function(req, res){
  Finisher.find({}, function(err, dbFinishers) {
    res.json({finishers: dbFinishers});
  })
});
...
router.get('/:id', function(req, res, next){
  Finisher.findById(req.params.id, function(err, dbFinisher) {
    ...
    if (!dbFinisher) { return next({status: 404, message: 'Not found'}); }
    res.json(dbFinisher);
  });
});
...
router.post('/', function(req, res, next) {
  ...
  if (!req.body.finisher) { return next({status:422, message: 'Missing arguments' }); }
