...
module.exports = function(socket, io) {
  ...
  // player asks if race is over
  socket.on('is race over', function(data) {
    data.finishers = game.getFinishers(data.raceId);
    console.log('rooom info from client + game.getFinishers: ', data);
    // ----------------room empties-------------
    if (game.allPlayers[data.room].length < 1) {
      // socket.leave(data.room);
      io.to(data.room).emit('race over', data);
    };
    socket.to('race not over', data)
  });
