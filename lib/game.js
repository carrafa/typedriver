...
module.exports = {
  ...
  startRace: function(data, callback) {
    if (this.allPlayers[data.room].length >= 2) {
  ...
  updatePlayer: function(data) {
    index = this.allPlayers[data.room].getIndexBy('id', data.id); // find player in array by ID
  ...
  playerFinishes: function(data) {
    var index = this.allPlayers[data.room].getIndexBy('id', data.userId)
  ...
  getFinishers: function(raceId) {
    var finishers = Finisher.find({
      ...
      console.log('db finishers: ', dbFinishers);
    });
  }
}
